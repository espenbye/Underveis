...
- **`Underveis.xcodeproj` is generated and git-ignored.** Never edit it by hand — change `project.yml`
...
- **No** `try!` / force-unwrap / `fatalError` in app code (one deliberate `fatalError` guards
  unrecoverable `ModelContainer` creation in `UnderveisApp`).
...
- **Auth:** no API key. Send `ET-Client-Name: espenbye-underveis` as an HTTP header _and_ in the
