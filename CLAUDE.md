...
- **`Underveis.xcodeproj` is generated but committed.** It's generated by xcodegen, yet checked in
  so **Xcode Cloud** can build it (the runners can't reach Homebrew/ghcr.io to install xcodegen). Never
  edit it by hand — change `project.yml`, run `make generate`, and **commit the regenerated project**.
